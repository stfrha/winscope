...
/*
20051003: These causes compile error in .NET environment. Should be safe to remove since 
CWsWaveDoc is never assigned as a message receiver.
...
*/
ON_COMMAND(ID_FILE_SETUPAUTOSAVE, OnFileSetupautosave)
END_MESSAGE_MAP()
...
BOOL CWsWaveDoc::OnNewDocument()
{
	...
	m_scopeDialog = new CScopeDialog("FRHA Scope Control Window", AfxGetMainWnd());
	m_scopeDialog->m_doc = this;
	m_setupAutosaveDlg.m_filenameBase = CString("AS_") + GetTitle();
	m_setupAutosaveDlg.m_autosaveActive = FALSE;
	m_setupAutosaveDlg.m_nextIndex = 1;
...
{
	MessageBox(NULL, (char*) lParam, "FRHA Scope Error (in doc)", MB_OK | MB_ICONEXCLAMATION);
	m_pCommPacket->commandType = WSC_TERMINATE_THREAD;
	return 0;
}
...
LRESULT	CWsWaveDoc::OnNewDataAvailable(WPARAM wParam, LPARAM lParam)
{
	...
	UpdateAllViews(NULL);
	// Autosave???
	if (m_setupAutosaveDlg.m_autosaveActive) {
		Autosave();
	}
...
{
	m_pCommPacket->commandType = WSC_TERMINATE_THREAD;
	MessageBox(NULL, "HW is not configured\nPlease configure now (Hw->Configure HW)", "FRHA Scope Error", MB_OK | MB_ICONEXCLAMATION);
	return 0;
}
...
{
	CFileDialog		fd(FALSE, ".csv", GetTitle().Left(GetTitle().GetLength()-4) + ".csv");

	if (fd.DoModal() == IDOK) {
		DoCSVExport(fd.GetPathName());
	}
}

void CWsWaveDoc::DoCSVExport(CString path)
{
	unsigned long	i;
	FILE*			of;
	...
	for (i=0 ; i<m_docSetting.m_bufferSize ; i++) {
		fprintf(of, "%03d\t%03d\t%03d\t%03d\n", *(m_probeA + i), *(m_probeB + i), *(m_podP + i), *(m_podQ + i));
	}
	fclose(of);
}
...
void CWsWaveDoc::OnFileSetupautosave()
{
	m_setupAutosaveDlg.DoModal();
}

void	CWsWaveDoc::Autosave( void )
{
	CString	indexS;
	indexS.Format("%d", m_setupAutosaveDlg.m_nextIndex++);
	CString	newTitle = m_setupAutosaveDlg.m_filenameBase + indexS;
	CString	newDoc = newTitle + ".wsw";

	CFile	f;
	f.Open(newDoc, CFile::modeCreate | CFile::modeWrite);
	CArchive ar(&f, CArchive::store);
	Serialize(ar);
	ar.Close();
	f.Close();

	if (m_setupAutosaveDlg.m_autosaveExport) {
		CString	newCsv = newTitle + ".csv";
		DoCSVExport(newCsv);
	}
}
