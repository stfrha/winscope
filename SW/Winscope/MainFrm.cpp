...
{
	MessageBox("FRHA Scope thread said hello", "Hello", MB_ICONINFORMATION | MB_OK);
	return 0;
}
...
LRESULT CMainFrame::OnScopeError(WPARAM wParam, LPARAM lParam)
{
	...
	MessageBox((char*) lParam, "FRHA Scope Error", MB_OK | MB_ICONEXCLAMATION);
	return 0;
}
...
{
	CFileDialog			fd(TRUE, ".bit", NULL, OFN_HIDEREADONLY | OFN_NOCHANGEDIR | OFN_OVERWRITEPROMPT | OFN_PATHMUSTEXIST, "XILINX Configuration file (*.bit)|*.bit||");
